<x-app-layout title="Add Route">
    <div class="px-4 py-8 sm:px-6 lg:px-8">
        <div class="mx-auto w-full max-w-3xl space-y-6">
            <div class="flex items-center justify-between gap-4">
                <div><h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100 md:text-3xl">Add Route</h1><p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Create a route for the selected account.</p></div>
                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route('routes.store') }}" class="space-y-5">
                        ...
                        <div>
                            ...
                            <x-input id="route_name" name="route_name" type="text" :value="old('route_name')" required />
                        </div>
                        <div>
                            ...
                            <select id="scheduled_day" name="scheduled_day" class="block w-full rounded-xl border-gray-300 bg-white px-4 py-3 text-sm text-gray-800 shadow-sm focus:border-violet-500 focus:ring-violet-500 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100" required>
                                ...
                                @foreach ($scheduledDayOptions as $scheduledDay)
                                    <option value="{{ $scheduledDay->value }}" @selected(old('scheduled_day') === $scheduledDay->value)>{{ $scheduledDay->displayLabel() }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="flex items-center justify-end gap-3">
                            <a href="{{ route('routes.index') }}" class="inline-flex items-center rounded-xl border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-50 dark:border-gray-600 dark:text-gray-200 dark:hover:bg-gray-700">Cancel</a>
                            <x-button>Create Route</x-button>
                        </div>
                    </form>
