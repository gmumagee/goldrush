    ...
    <div class="px-4 py-8 sm:px-6 lg:px-8">
        <div class="mx-auto w-full max-w-3xl space-y-6">
            <div class="flex items-center justify-between gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100 md:text-3xl">Add Transaction</h1>
                    <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Create a bin-level transaction for the selected account.</p>
                </div>
                    ...
                    <form method="POST" action="{{ route('transactions.store') }}" class="space-y-5">
                        ...
                        <div class="grid gap-5 md:grid-cols-3">
                            <div>
                                ...
                                <select id="transaction_type" name="transaction_type" class="block w-full rounded-xl border-gray-300 bg-white px-4 py-3 text-sm text-gray-800 shadow-sm focus:border-violet-500 focus:ring-violet-500 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100" required>
                                    ...
                                    @foreach ($transactionTypes as $type)
                                        <option value="{{ $type }}" @selected(old('transaction_type') === $type)>{{ ucfirst($type) }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <x-input id="quantity" name="quantity" type="number" :value="old('quantity')" required />
                            </div>

                            <div>
                                <x-label for="spoilage" value="Spoilage" />
                                {{-- Keep spoilage editable for manual count transactions without affecting non-count transaction types. --}}
                                <x-input id="spoilage" name="spoilage" type="number" min="0" step="1" :value="old('spoilage', 0)" />
                                <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">Use this only for Count transactions. Leave 0 for Fill, Waste, Remove, or Adjustment.</p>
                            </div>
                        </div>
                        ...
                        <div class="grid gap-5 md:grid-cols-3">
                            <div>
                                ...
                                <x-input id="price" name="price" type="number" min="0" step="0.01" :value="old('price')" />
                            </div>
                            ...
                            <div>
                                ...
                                <x-input id="transaction_time" name="transaction_time" type="text" placeholder="HH:MM:SS" :value="old('transaction_time', \App\Support\AppDateTime::inputTime(now()))" required />
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-end gap-3">
                            ...
                            <x-button>Create Transaction</x-button>
                        </div>
                    </form>
